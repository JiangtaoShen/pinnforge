...
GRID_CSV = HERE / "eval_grid.csv"   # t, x — public: where to predict
REF_VALUES = "ref_values.npy"       # the answers; service side only

def _task_name():
    """Task this run is of — `runs/ks_1` -> `ks`. Env wins when set."""
...
def score_predictions(fields, ref_dir):
    """Service side: relative L2 on u over the whole reference grid."""
    import numpy as np

    gt = np.load(Path(ref_dir) / REF_VALUES, allow_pickle=False)[:, 0]
    u = np.asarray(fields["u"], dtype=np.float64).reshape(-1)
    if u.shape != gt.shape:
        return {"error": f"prediction shape {u.shape} != reference {gt.shape}"}
    if not np.all(np.isfinite(u)):
        return {"rRMSE": None, "MSE": None, "error": "non-finite predictions"}
    diff = u - gt
    return {"rRMSE": float(np.linalg.norm(diff) / np.linalg.norm(gt)),
            "MSE": float(np.mean(diff ** 2)), "n_scored": int(gt.size)}
...
def worker_eval(code_path: str, params_path: str) -> int:
    ...
    try:
        ...
        np.savez(pred_path, u=u_pred.reshape(-1))
